...
public class FormBarang extends JFrame {
    ...
    void simpanData(Barang B) {
        try {
            ...
            String sql = "INSERT INTO tbl_barang (kode_barang, nama_barang, harga_barang, stok_barang) " +
                    "VALUES ('" + B.getKodeBarang() + "', '" + B.getNamaBarang() + "', " + B.getHargaBarang() + ", "
                    + B.getStokBarang() + ")";
            int result = st.executeUpdate(sql);
            cn.close();
            JOptionPane.showMessageDialog(null, "Data Berhasil Disimpan",
                    "Info Proses", JOptionPane.INFORMATION_MESSAGE);
            String[] data = { B.getKodeBarang(), B.getNamaBarang(), String.valueOf(B.getHargaBarang()),
                    String.valueOf(B.getStokBarang()) };
            df.addRow(data);
        } catch (SQLException ex) {
            ex.printStackTrace();
        }
    ...
    void ubahData(Barang B, String kode_barang) {
        try {
            Connection cn = new ConnectDB().getConnect();
            Statement st = cn.createStatement();
            String sql = "UPDATE tbl_barang SET kode_barang='" + B.getKodeBarang() + "', nama_barang='"
                    + B.getNamaBarang() + "', harga_barang=" + B.getHargaBarang() + ", stok_barang=" + B.getStokBarang()
